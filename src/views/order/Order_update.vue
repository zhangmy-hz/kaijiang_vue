  ...
  <div>
    ...
    <el-card
      ...
      <el-table
        ...
        :data="order_li" :max-height="tableHeight"
      ...
      >
        ...
        <el-table-column
          label="类型" width="80px" prop="order_type">
          <template slot-scope="scope" >
            <el-select v-if="scope.row.update_status === 1"
        ...
        <el-table-column label="ERP单号" width="100px">
          <template slot-scope="scope">
            <el-input v-if="scope.row.update_status === 1" style="width: 100px;margin-left: -10px"
              v-model="scope.row.order_code" size="small"/>
            <span v-if="scope.row.update_status !== 1">{{scope.row.order_code}}</span>
          </template>
        </el-table-column>
        <el-table-column label="SKU编码" width="90px">
          <template slot-scope="scope">
            <el-autocomplete v-if="scope.row.update_status === 1"
        ...
        <el-table-column label="产品名称" width="140px" prop="item_name" />
        <el-table-column label="名字" width="140px">
          <template slot-scope="scope">
            <el-input v-if="scope.row.update_status === 1" style="width: 140px;margin-left: -10px"
              v-model="scope.row.order_name" size="small"/>
            <span v-if="scope.row.update_status !== 1">{{scope.row.order_name}}</span>
          </template>
        </el-table-column>
        <el-table-column label="单位" width="80px" prop="unit" />
        <el-table-column label="分类" width="90px" prop="skutype" />
        <el-table-column label="订单数" width="60px">
          <template slot-scope="scope">
        ...
        <el-table-column label="金额" width="60px">
          <template slot-scope="scope">
            <el-input v-if="scope.row.update_status === 1" style="margin-left: -10px;width: 60px"
              v-model="scope.row.amount" type="number"
              size="small"
            />
        ...
        >
          <template slot-scope="scope">
            <el-autocomplete v-if="scope.row.update_status === 1"
              ...
              size="small" style="width: 150px;margin-left: -10px;"
              class="el-input"
        ...
        >
          <template slot-scope="scope">
            <el-autocomplete v-if="scope.row.update_status === 1"
              ...
              size="small" style="width: 180px;margin-left: -10px"
              class="el-input"
        ...
        <el-table-column label="包装方式" width="128px">
          <template slot-scope="scope">
            <el-autocomplete v-if="scope.row.update_status === 1" style="width: 180px;margin-left: -10px"
        ...
        <el-table-column label="链长" width="128px">
          <template slot-scope="scope">
            <el-autocomplete style="width: 180px;margin-left: -10px"  v-if="scope.row.update_status === 1"
        ...
        >
          <template slot-scope="scope">
            <el-input  v-if="scope.row.update_status === 1"
        ...
        >
          <template slot-scope="scope">
            <el-date-picker v-if="scope.row.update_status === 1"
...
export default {
  ...
  methods: {
    ...
    preview_order() { //订单预览界面
      ...
      this.$router.push({ path: '/2-1-4', query: { order: this.addForm.id } })// 打开预览报表
    },
    get_data() {
      var aData = new Date()
            ...
            that.$http.post('order_update/', { order_tou: that.addForm, order_shen: that.order_li, delete_li: that.delete_li,user_name:that.update_user_name })
              .then(function (res) {
                if (res.data === 'OK') {
                  ...
                  that.delete_li = [] // 删除的集合
                  that.page_get()  //页面数据重新加载
                }else if (res.data === 'OJB'){
                  that.$message.success('订单保存成功')
                  that.delete_li = [] // 删除的集合
                  that.page_get()  //已经有订单批次了.重新加载页面
                  that.$notify({
                    title: '提示',
