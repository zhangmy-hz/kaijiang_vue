<template>
  <div>
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>包装方式</el-breadcrumb-item>
    </el-breadcrumb>
    <el-card style="margin-top: 10px;width: 700px">
      <el-row :gutter="20" style="margin-top: -10px;margin-bottom: 5px">
        <el-col class="el-row" :span="4">
          <el-button type="primary" size="small" @click="addVisible = true" v-if="role_tree_list.indexOf('1-1-1') !==-1">
            添加包装
          </el-button>
        </el-col>
      </el-row>
      ...
      <el-table :data="color_li" size="small" border :header-row-style="{'height': '15px'}" :row-style="{'height': '15px'}" :stripe="true">
        ...
        <!--新增颜色-->
        <el-dialog
          title="新增包装" :visible.sync="addVisible" width="30%" :append-to-body="true" @close="handleClose">
          <span>
            <el-form ref="Formref" :model="addForm" :rules="formrules" label-width="100px">
              ...
              <el-form-item label="包装名" prop="name">
...
export default {
  ...
  methods: {
    color_list() {
      var that = this
      this.$http.get('pack_method/').then(function(res) {
        // console.log(res)
    ...
    add_color() {
      ...
      this.$refs.Formref.validate(valid => {
        if (valid) {
          this.$http
            .post('add_packing/', { form_data: this.addForm })
            .then(function(res) {
              if (res.data === 'OK') {
    ...
    update_color() {
      ...
      this.$refs.upFormref.validate(valid => {
        if (valid) {
          this.$http
            .post('update_packing/', { form_data: this.updateForm })
            .then(function(res) {
              if (res.data === 'OK') {
    ...
    showEditDialog(name) {
      ...
      this.$http.post('packing_up_select/', { id: name }).then(function(res) {
        // console.log(res)
    ...
    delete_color(name) { // 删除颜色，并提示
      var that = this
      this.$confirm('此操作将永久删包装, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(function (res) {
        if (res === 'confirm') {
          that.$http.post('delete_packing/', { color_id: name })
            .then(function (res) {
