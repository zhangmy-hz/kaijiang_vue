  ...
  <div>
    ...
    <el-card
      ...
      <el-row :gutter="58" style="margin-top: -10px">
        ...
        <el-col class="el-row" :span="1">
          <el-button size="small" @click="printer">
            打印
          </el-button>
        </el-col>
      </el-row>
      <el-form ref="Formref" :model="addForm" :inline="true" :rules="formrules" label-width="80px" size="mini" style="margin-top: -30px">
        <div style="width:100%">
      ...
      <el-table
        ...
        <el-table-column label="金额" width="70px">
          <template slot-scope="scope">
            <el-input v-model="scope.row.total" disabled size="small"  style="width: 70px;margin-left: -10px"/>
          </template>
        </el-table-column>
    ...
    <div id="printTest" style="display: none">
      <div>
        <table border="1" cellspacing="0" cellpadding="0" style="text-align: center;table-layout: fixed;border-collapse: collapse;font-size: 12pt">
          <thead>
          <tr>
            <th colspan="5">单号:{{addForm.order_code}}   供应商:{{addForm.supplier_ware}}   日期:{{addForm.date}}   备注:{{addForm.note}}</th>
            <th colspan="1"><span tdata='pageNO'>第##页</span>/<span tdata='pageCount'>共##页</span></th>
          </tr>
          <tr>
            <th width="10%">材料编号</th>
            <th width="30%">材料名称</th>
          ...
          <tbody>
          <tr v-for="item in ruku_li" :key="item.id">
...
export default {
  ...
  methods: {
    ...
    printer() {
      var that = this
      this.$confirm('保存的内容才会打印,确认打印？', '提示', {
